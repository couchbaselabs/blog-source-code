...
XDCR supports flexible replication of data between different clusters.  The MCA client can automatically redirect traffic
to these separate clusters based on library configuration, relieving the application developer of managing much of the
complexity involved.
...
Webinar tutorial - link:https://github.com/couchbaselabs/xdcr-java-sample/blob/master/Tutorial.adoc[Setting up clusters step-by-step, with XDCR]
...
Currently the MCA client adds three other groups of features:

. The ability to prioritize a list of clusters.
. Ways to supply rules on what constitutes a failure.
. A management interface.

We'll discuss the first two using the code sample.  Management happens via a JMX interface.  This allows administrators to switch nodes in the priority list.  It falls outside the scope of this post.
...
You can find the complete code, along with build instructions and other materials link:https://github.com/couchbaselabs/xdcr-java-sample[on GitHub]. Here's an
example invocation running against two 3-node clusters with 10 threads updating documents.

[source,bash]
----
java -jar xdcr-1.0.jar -c 172.23.123.4,172.23.123.5,172.23.123.6:172.23.122.55,172.23.122.56,172.23.122.57 -b my_bucket -i user -p password -u 10 -l statistics
----
...
link:https://github.com/couchbaselabs/xdcr-java-sample/blob/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[ClusterBasics] class.
...
and creates a cluster spec for each cluster.

[source,java,indent=0]
----
include::https://raw.githubusercontent.com/couchbaselabs/xdcr-java-sample/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[tag=cluster-spec]
----
...
[source,java,indent=0]
----
include::https://raw.githubusercontent.com/couchbaselabs/xdcr-java-sample/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[tag=services]
----
...
[source,java,indent=0]
----
include::https://raw.githubusercontent.com/couchbaselabs/xdcr-java-sample/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[tag=coordinator]
----
...
<5> Timeout period to wait before marking failure
<6> Select the behavior when the end of a topology is reached.  Alternatives include staying at the end cluster, or reversing through the list.
...
[source,java,indent=0]
----
include::https://raw.githubusercontent.com/couchbaselabs/xdcr-java-sample/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[tag=failure-detector]
----
...
[source,java,indent=0]
----
include::https://raw.githubusercontent.com/couchbaselabs/xdcr-java-sample/master/src/main/java/com/couchbase/server/xdcr/ClusterBasics.java[tag=client]
----
...
find out more and get access to the client, talk to a Couchbase sales representative.  You can contact me for more information also,
as described below.
