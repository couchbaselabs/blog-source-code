:imagesdir: images
:meta-description: Machine Learning APIs from Nexosis can be used directly from Couchbase using the CURL function available in N1QL. In this post, we'll use machine learning to classify Amazon reviews.
...
Let's test the model out with REST. I'll be making a POST request to the /predict url using the modelId like: POST https://ml.nexosis.com/v1/models/<model ID guid>/predict 

The body of this request will contain the Amazon review that I want to get Nexosis's opinion on whether or not it's a positive review. I'll give it the text of a 1-star review.
...
In Couchbase, I've created a bucket with some reviews I pulled manually from Amazon (and I executed `CREATE PRIMARY INDEX on reviews`).
...
Notice that the `body` is pulling the text directly from the review document in Couchbase. The only thing being selected is the entire response from Nexosis, which looks similar to this:
...
And here's the results (I truncated the actual text for the blog):
...
What is interesting is that I also decided to give it a 3-star "neutral" review. Note that the training set had none of these, so this may not be a useful result, but I was curious. It classified it as a negative review. Just based on the language alone this is probably accurate. This might actually be a helpful tool to look at ostensibly "neutral" reviews and discern if they are leaning one way or the other.
...
If you are interested in Nexosis, check out the link:https://nexosis.com/use-case[Nexosis Use Cases] page to see how Nexosis can help you or your business.
