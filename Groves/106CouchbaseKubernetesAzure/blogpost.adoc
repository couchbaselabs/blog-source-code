...
Clicking this will open up a shell command prompt window right in your browser. The first time you do this, you will be prompted to decided between Bash and PowerShell. I decided to use PowerShell. _(If you are more comfortable with Bash, then you should still be able to follow along)_. You will need to setup a storage account the first time you use the Cloud Shell. This will take several minutes, but you only need to do it the first time.
...
This command will print out a list of your accounts in JSON format. Check out the `isDefault` field in the JSON. For instance, mine had my "Pay as you go" account as the default (which I don't want, because I have free credit on my MSDN account).
...
You can use the subscription name or the ID. 
...
Before we can create a Kubernetes cluster with a Couchbase cluster, let's lay some groundwork.
...
If you're using Cloud Shell, you probably won't need the following steps. But if you're trying to use your local Windows PowerShell, you might also need to take the following steps:

* Do you have the ContainerService registered? You can find out by running `az provider list | ConvertFrom-Json | Format-Wide`
* Start ContainerService registration: `az provider register -n Microsoft.ContainerService
* Monitor the progress of ContainerService registration: `az provider show -n Microsoft.ContainerService` (wait until "registrationState" is not "Registering" and becomes "Registered")
...
_(AKS cluster name can only contain letter,numbers,dashes)._
...
* Each node is a DS1_v2 (Single CPU, 3.5gb RAM machines, which are inexpensive and fine for this demo, but you will probably want to use a more powerful machine for Couchbase in production)
...
Start by creating the operator with `kubectl`:
...
When that's finished, you can view the deployements with `kubectl get deployments`.
...
At this point, the "pods" will start to be created on AKS. You can check out the status with: `kubectl get pods`.

image:10606-get-pods.png[get pods]
image:10605-kubectl-get-pods.png[kubectl get pods]

This will take a few minutes to complete.

## Access the AKS cluster

The Couchbase cluster is now created and can be accessed from other pods in the cluster. One other way you can access it, just to verify it's working, is with port forwarding. Run this command locally:
...
When you do this, your localhost on port 8091 will be forwarded to the cb-example-0000 pod on port 8091. Which means that if you open http://localhost:8091 in your local browser, you'll be directed to the Couchbase cluster running in AKS on Azure.

image:10607-couchbase-cluster.png[Couchbase cluster]

An easy way to see AKS and Kubernetes in action is to remove the bucket. The Couchbase operator will see this, and recreate the bucket.
