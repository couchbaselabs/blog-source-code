...
:slug: Geospatial-Search-ASPNET-Core-Aurelia-Google-Maps
:focus-keyword: geospatial
:categories: Couchbase Server, .NET
:tags: geospatial, search, geospatial search, asp.net, c#, aurelia, google maps, typescript, javascript
...
In this post, I'm going to demonstrate the geospatial search capabilities of Couchbase Server by creating a web-based UI that performs searches. Whenever I think of geospatial searches, I think about Yelp, which is great at helping me find restaurants in a specific area.
...
1. At the command line: `dotnet new aurelia`. This assumes that you have link:https://dotnet.github.io/[.NET Core] installed and that you've installed a link:https://www.hanselman.com/blog/dotnetNewAngularAndDotnetNewReact.aspx[SPA template for Aurelia]. You can also go with Angular or React if you'd like, but I really like Aurelia, and I think you should give it a chance.

2. The above command will create a shell of an ASP.NET Core project. In this blog post, I'm not going to use Razor. I'm just using ASP.NET as a backend for REST API endpoints. *Note that Geospatial Search is not a .NET-only feature: you can use it with the link:https://www.couchbase.com/products/developer-sdk[other Couchbase SDKs] like Node.js, Java, etc.*
...
4. I opened this project in Visual Studio 2017. I added link:https://blog.couchbase.com/dependency-injection-aspnet-couchbase/[Couchbase.Extensions.DependencyInjection]. You don't have to use this extension but it makes things easier.

5. I installed Couchbase Server 5.5. I setup the travel-sample bucket. I created a user "matt" with full access to that bucket.
...
I named my index "mygeoindex". It will be on the travel-sample bucket, so just select that one.
...
Next, let's move over to ASP.NET. I'll create two endpoints. One endpoint will demonstrate the *bounded box* search method, and the other will demonstrate the *distance* search method.

I'll need a Couchbase bucket object to execute the queries. Follow the examples in the post linked above for dependency injection or check out the source code on Github if you've never done this before.

=== Bounded Box

A "bounded box" search means that you define a box on a map, and you want to search for points of interest that are inside of that box. You only need two points to define a box: the top right corner coordinates and the bottom left corner coordinates. (Coordinates are latitude and longitude).
...
To create a bounded box geospatial query, use the `GeoBoundingBoxQuery` class available in the .NET SDK. I'll do this inside of a POST method with the above `BoxSearch` class as a parameter.
...
All I need to return for now is a list of the results: each hotel's coordinates and the hotel's name & location. I created a `GeoSearchResult` class for this.
...
Note that I've constructed this class to match the Google Maps plugin that I'll be using later.
...
=== Bounded Box search component

On this component, you'll need to first click a spot on the map. This will set the first coordinate (top left).
...
My implementation is very bare bones. No fancy graphics and no validation of the second coordinate being to the bottom right of the first.

Once you've selected two coordinates, a search button will appear. Click that to post these coordinates to the endpoint created earlier.
...
This will be similar to the bounding box UI. This time, you only need to click a single point on the map. But, you will need to type in a distance (in miles).
...
If you need help or have questions, please check out the link:https://forums.couchbase.com/c/couchbase-server[Couchbase Server forums].
