...
There's still something missing from this result. Where are those GUIDs that we used for the document keys? Couchbase doesn't store them as data; it stores them as *metadata*. SQL{plus}{plus} provides the `META()` function to query metadata. Use `META().id` like this:
...
To execute SQL{plus}{plus}, you need an object of type `Cluster`. SQL{plus}{plus} runs at the cluster level (not bucket, or scope, or collection, since it may need to `JOIN`/`UNION` between them). You could add link:https://docs.couchbase.com/dotnet-sdk/current/howtos/managing-connections.html#injecting-the-couchbase-cluster[`ClusterProvider`] as a constructor parameter. If this endpoint was going to _only_ work with SQL{plus}{plus}, that would be a good idea. However, let's stick with `BucketProvider` for now. We have an object of type `BucketProvider`. From that object, you can get a object of type `Cluster`:
...
In a relational database, this would likely require an `ALTER` to add a new column. Depending on the size of your data, it could require some downtime, or possibly a lot of NULL values. In a JSON database, we don't need to tell the database anything about a new field.
