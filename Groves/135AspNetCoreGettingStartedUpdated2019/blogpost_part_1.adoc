...
Inside that bucket, there's a _default scope, so I'll use that. And inside that scope, I've created a "wishlist" collection. (link:https://docs.couchbase.com/cloud/clusters/data-service/scopes-collections.html[Learn more about scopes and collections in Capella])
...
Finally, link:https://docs.couchbase.com/cloud/clusters/allow-ip-address.html[whitelist the IP address] that you will be compiling and running your app from, so that you can connect.
...
I'm also going to add the Couchbase.Extensions.DependencyInjection NuGet package (Couchbase.Extensions.DependencyInjection). This will provide extensions method to easily add Couchbase capabilities to ASP.NET Core's built-in dependency injection.
