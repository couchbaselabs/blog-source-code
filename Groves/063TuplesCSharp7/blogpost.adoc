...
:title: C# Tuples: New to C# 7
:slug: csharp-tuples-new-to-csharp-7
...
The link://[source code that I use for this blog post is available on GitHub] for you to try out if you'd like.
...
The `GenerateRandomFilm` method returns a `Film` object with some random-ish values (check out the link://[GitHub source] for details). A tuple of `(string, Film)` is passed to `InsertTuple`. The Couchbase .NET SDK takes it from there and inserts a document with the appropriate key/value.
