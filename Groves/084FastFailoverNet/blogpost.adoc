...
To try this out requires a bit of setup and preparation.

First, you'll need to create a 3-node (at least) Couchbase Server cluster. There are a number of ways to do this, including link:https://github.com/couchbaselabs/vagrants[Vagrants], Virtual Machines, actual machines, link:https://blog.couchbase.com/azure-getting-started-easy-free/[Azure], and more. Having at least three nodes will make it possibly to demonstrate fast failover.

I chose to use Docker for this blog post. I blogged about how to link:https://blog.couchbase.com/docker-and-asp-net-core-with-couchbase-server/[create a Couchbase Cluster on Docker and access it with a .NET Core application](don't forget the bridge network!). So, I just followed those same instructions again. The main difference is that I used a console application instead of an ASP.NET application (which you can read more about later in this post).

image:08401-three-couchbase-nodes.png[Three Couchbase Server nodes]

I used the link:https://hub.docker.com/r/couchbase/server/tags/[Couchbase Server 5.0.0-beta2 image from Docker Hub], but by the time you read this post, an official release of Couchbase Server 5.0 should be link:https://hub.docker.com/r/library/couchbase/tags/[available on the official docker Couchbase repository].

Next, I created a bucket called "mybucket". Make sure to enable replication of 2 (assuming you're using a 3 node cluster).
...
For this demonstration, I want to setup a bunch of documents that I will later be reading from, repeatedly. I wrote a loop to create some arbitrary number of documents, that each have a key like "documentKey[num]" (e.g. "documentKey1", "documentKey2", etc).
...
Now that there are 50 documents with well-known keys, the rest of the program will be continuously looping. Each loop iteration will attempt to retreive all 50 documents.
