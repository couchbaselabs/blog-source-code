...
The link:https://github.com/couchbaselabs/Couchbase.Extensions[Couchbase.Extensions (GitHub)] project aims to make working with Couchbase Server and .NET Core simpler.

You can add it to your ASP.NET Core project with NuGet, via Package Manager: `Install-Package Couchbase.Extensions.Caching -Version 1.0.0-beta2`, or with the UI, or you can use the .NET command line: `dotnet add package Couchbase.Extensions.Caching --version 1.0.0-beta2`.
...
Lastly, in the sample code, I simply print out the `ViewData["Message"]` in the Razor view. It should look something like this:
...
First, I print out the key to the Razor view:
...
Finally, don't forget to cleanup the resources used by Couchbase for distributed caching. One easy way to do this is with the `ApplicationStopped` event. You can wire this up in `Startup`:
