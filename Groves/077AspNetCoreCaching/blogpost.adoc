...
:slug: Distributed-caching-ASP-NET-Core-Couchbase
...
If you are using Couchbase Server 5.0, you'll also need to create a user with permissions (Data Writer[cachebucket] and Data Reader[cachebucket]) on that bucket. To keep things simple, create a user that has the same name as the bucket (e.g. "cachebucket").

== Distributed Caching with Couchbase.Extensions

The link:https://github.com/couchbaselabs/Couchbase.Extensions[Couchbase.Extensions (GitHub)] project aims to make working with Couchbase Server and .NET Core simpler. You can add it to your ASP.NET Core project with NuGet, via Package Manager: `Install-Package Couchbase.Extensions.Caching -Version 1.0.0-beta2`, or with the UI, or you can use the .NET command line: `dotnet add package Couchbase.Extensions.Caching --version 1.0.0-beta2`.
...
This will make the Caching namespace available, and specifically the `AddDistributedCouchbaseCache` extension method for `IServiceCollection`. Simply call that extension method from within the `ConfigureServices` method:
...
Now that you have a distributed cache setup with Couchbase in your ASP.NET Core project, you can use `IDistributedCache` elsewhere in your project.

A simple example would be to use it directly in a controller. It will be injected into constructors as you need it:
...
You can use the `GetString` `SetString` methods to retrieve/set a string value in the cache.
...
This would appear in the "cachebucket" bucket as a binary value (not the JSON you're accustomed to).
...
Next, in the sample, I generate a random string to use as a cache key, and a random generated instance of `MyPoco`. I store them in the cache using the `Set<>` method:
...
I print out the key to the Razor view:

image:07704-cached-poco.png[Cached POCO output to Razor]

You can use this key to look up the value in Couchbase:

image:07705-cached-poco-couchbase-json.png[Cached POCO in Couchbase]

Notice that it's been serialized to JSON. This not only means that you can read it, but you can also query it with N1QL (if you need to).

== Distributed cache with expiration
...
== Tearing down the distributed cache

Don't forget to cleanup the resources used by Couchbase for the distributed cache. One easy way to do this is with the `ApplicationStopped` event. You can wire this up in `Startup`:
...
Using Couchbase Server as a distributed cache in your ASP.NET Core application is a great way to improve performance and scalability to your application. These kind of "engagement" use cases are what Couchbase Server excels at. To see what else Couchbase customers are doing, check out the link:https://www.couchbase.com/customers[Couchbase Customers] page.
...
As always, you can also reach me by leaving a comment below or finding me on link:https://twitter.com/mgroves[Twitter @mgroves].
