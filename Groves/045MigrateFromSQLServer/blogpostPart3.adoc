...
* *Kafka* is an open source data streaming tool. Some of the link:http://kafka.apache.org/uses[popular use cases for Kafka] include messaging, website activity tracking, metrics, and more. 
...
link:https://www.couchbase.com/bigdata[Couchbase has connectors] that support each of these three popular tools.

Finally, link:https://developer.couchbase.com/documentation/server/4.5/analytics/introduction.html[*Couchbase Analytics* is currently in developer preview]. It is intended as a data management engine that runs parallel to Couchbase Server. It's a developer preview, and is not yet recommended to be used in production, but you can link:https://www.couchbase.com/downloads[download Couchbase Analytics and Kafka, Spark, Hadoop extensions (click the Extensions tab) and try them out].
