...
Using a link:https://blog.couchbase.com/serverless-architecture-cloud-computing/[serverless architecture] for a chatbot makes sense. Chatbot usage may be sporadic. Usage may peak and drop at various times of the day. By using serverless, you'll only be paying for the resources and time that you need.

_If you want to follow along, all the source code for this blog post is link://[available on Github]._
...
Also, link:https://developers.viber.com/docs/api/rest-bot-api/[Viber's REST API] is simple and well documented. Between the user of serverless architecture and Viber's API, I couldn't believe how fast I went from 0 to chatbot.

== Setup

First, You'll need to start by link:https://viber.github.io/docs/general/get-started/[creating a bot in Viber (you'll need a Viber account at some point). Viber will give you an API key that will something like `30a6470a1c67d66f-4207550bd0f024fa-c4cacb89afc04094`. This is how you will authenticate to the Viber API.
...
Finally, I setup a Couchbase cluster on Azure. I didn't create anything fancy, just a single node. link:https://blog.couchbase.com/azure-getting-started-easy-free/[Getting started with Couchbase and Azure is easy and free]. (You can even use the "Test Drive" button to get 3 hours of Couchbase Server without expending any Azure credit).
...
`ProcessMessage` will analyze the text of the message and figure out what to do in response. You can check out the complete code on link://[Github], but here's a brief snippet to give you the idea:
...
That's all there is to it. To test out the bot, I used my Viber app for Android on my phone as well as my wife's.

[ image of viber conversation ]

Here's a recap of the benefits of this approach to creating a chatbot:

* The link:https://blog.couchbase.com/serverless-architecture-cloud-computing/[serverless approach] is a good way to control costs of a chatbot, whether it's Viber or some other messaging platform because of the potential for sporadic and cyclic use.
...
This was really fun, and I could definitely get carried away playing with this new chatbot. It could analyze images, tell jokes, look up all kinds of information, sell products and services, or any number of useful operations. I would love to hear what you're building a chatbot to do (please leave a comment or contact me on link:https://twitter.com/mgroves[Twitter @mgroves])!