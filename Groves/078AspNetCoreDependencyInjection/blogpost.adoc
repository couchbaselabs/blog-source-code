...
You can add it to your ASP.NET Core project with NuGet, via Package Manager: `Install-Package Couchbase.Extensions.DependencyInjection -Version 1.0.2`, or with the NuGet UI, or you can use the .NET command line: `dotnet add package Couchbase.Extensions.DependencyInjection --version 1.0.2` (version 1.0.2 is the latest version at the time of writing).
...
Next, let's do a simple `Get` operation on a well-known document in "travel-sample" just to show dependency injection in action. I'll make a change to the generated `About` action method, and write out the equipment number of the Columbus to Chicago flight for United:
...
You can use the dependency injection for a specific bucket instead of having to specify the name each time.
...
Now, the `ITravelSampleBucketProvider` can be injected instead of the more general provider.
...
Until this point, we've only used dependency injection on Controllers. Dependency injection starts to pay dividends when you have a more complex object graph that goes deeper than the Controller level.
...
If I were to instantiate `ComplexService` manually, I would have to instantiate at least three objects. It would look something like: `new ComplexService(new BucketProvider(), new MyEmailService()`. That's a lot that I have to keep track of, and if any dependencies change, it's a lot of manual maintenance.
...
And, of course, ASP.NET Core is already able to instantiate controllers like `ApproveController` directly.

Finally, when `ApproveController` is instantiated, ASP.NET Core will know how to create and inject `ComplexService` automatically. The end result:
...
Couchbase.Extensions.DependencyInjection makes it easier for you to pair dependency injection and Couchbase configuration with ASP.NET Core.
