...
This is fine for demos and blog posts, but in reality, you probably want to use a configuration file.
...
Next, let's do a simple `Get` operation on a well-known document in "travel-sample" just to show that this is working. I'll make a change to the generated `About` action method, and write out the equipment number of the Columbus to Chicago flight for United (a route that I fly often):
...
Until this point, we've only used dependency injection on Controllers. In a real project, dependency injection really starts to be valuable when you have a more complex object graph that goes deeper than the Controller level.
...
Instead, I can just tell ASP.NET Core how to use dependency injection to do all this for me. Back in `Startup`:
