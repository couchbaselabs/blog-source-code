...
Kafka is a streaming platform that can be used to stream records to (Kafka sink) and from (Kafka source) data centers. Couchbase has created and supports a link:https://docs.couchbase.com/kafka-connector/3.4/index.html[Kafka connector] that allows you to easily use Couchbase as a source or a sink.

I've been working on a complete tutorial for a link:https://www.couchbase.com/solutions/customer-360[Customer 360] use case. Here's a high level diagram:
...
* link:https://www.docker.com/[Docker]. You will need to have Docker installed to complete this tutorial. _You may eventually want to move all of this into docker-compose or (more likely) link:https://docs.couchbase.com/operator/current/overview.html[Kubernetes]._
...
At this point, you can connect to the MySQL database. The Debezium tutorial provides another Docker image for that, or you can use the MySQL tool of your choice. You don't need to do this right now, but eventually you'll want to connect to it and insert/update data to test the process end-to-end.

== Preparing Kafka Connectors

At this point in my journey, I must diverge slightly from the Debezium tutorial. That tutorial shows you how to start a Debezium MySQL Connector (with another Docker image and a REST request). However, I want to introduce the Couchbase Kafka Connector at this point.
...
Now that you've completed this tutorial, there are a few different directions you can go to continue exploring.

* Orchestration: this sample contains a lot of Docker images that are run manually. In a production environment, you'd likely want to orchestrate and scale all these pieces with something like Kubernetes. There is a link:https://docs.couchbase.com/operator/current/overview.html[Kubernetes operator for Couchbase Enterprise].
