...
This tutorial contains some steps that are Azure AKS specific, but the bulk of this material can be applied to any Kubernetes cluster.
...
hellomicroservice                      dev                 394211295bfc        30 minutes ago      207MB
mattstwitchbotweb                      latest              417e7194114c        2 weeks ago         215MB
mattgroves/mattstwitchbotweb           latest              417e7194114c        2 weeks ago         215MB
----
...
Now that you have published a Docker image, we can start to write the YAML files for a Kubernetes deployment. This tutorial will not cover everything there is to know about Kubernetes, but it will point out noteworthy sections as much as possible.

Let's start by creating a YAML file for the ASP.NET Core microservice itself:
...
* `spec.replicas`: This tells Kubernetes how many pods of this service to create. To start with, 1 is fine, but as you scale, you will need more pods (and a LoadBalancer, which will be covered later).
...
* `env`: These are variables that will be set as environment variables. I'm passing a Username, Password, and ConnectionString to the ASP.NET Core microservice, much like was done in the previous blog post. Most on the Couchbase__ConnectionString and DNS SRV later.
...
When the service is created, it will create an external IP address. You can view service information with `kubectl get services`. Open that IP address in a web browser, and it will be load balanced access to the pods in your deployment. As you scale up or down, the service will remain the same and continue to provide access.
