...
So, with a relational database, you might use an OR/M like NHibernate or Entity Framework. The way this mapping is represented in Entity Framework* like this:
...
* Model users as individual documents
* Model updates as individual documents that reference a user
* Model friends as an array of keys that reference other users

With two users, two updates, that would look like 4 documents in Couchbase:
...
You can get the link:https://github.com/couchbaselabs/blog-source-code/tree/master/Groves/045MigrateFromSQLServer1/src[source code for the entire blog series on GitHub now]. If you have questions about various parts of that code, feel free to leave a comment below, or open an issue on GitHub.
