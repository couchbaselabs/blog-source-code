...
I'm going to cover data modeling, [links when posts are published, put them in numbered list] the data itself, and applications using the data. The goal is to lay down some general guidelines that you can apply to your application planning and design.
...
* *Scalability*: Couchbase Server is a link:http://developer.couchbase.com/documentation/server/current/architecture/architecture-intro.html?utm_source=blogs&utm_medium=link&utm_campaign=blogs[distributed database], which allows you to scale out (and scale back in) capacity by just racking up commodity hardware. [mention some of the drawbacks of scalaiblity with SQL server]
* *Flexibility*: Some data fits nicely in a relational model, but some data can benefit from the link:http://developer.couchbase.com/documentation/server/current/data-modeling/concepts-data-modeling-intro.html?utm_source=blogs&utm_medium=link&utm_campaign=blogs[flexibility of using JSON]. [mention schema maintenance, SQL Server restrictions]
...
The ease or difficulty of a transition varies widely based on a number of factors. In some cases it may be extremely easy; in some cases it will be time-consuming and difficult; in some (shrinking number of) cases it may not even be a good idea.
...
| Server | Cluster 
| Database | Bucket
| Row(s) from multiple tables | Document
...
In Couchbase, using a denormalization strategy, a shopping cart and the items in it would be reprsented by a single document.
...
These classes would still make sense with Couchbase, so you can reuse them or design them this way. But this design does not match up to a relational table in a direct way.

So, with a relational database, you might use an OR/M like NHibernate or Entity Framework. The way this mapping is represented in an OR/M* would look like this:
...
_*This is psuedo-code that is meant to represent EF, NH, Linq2SQL, or similar OR/Ms_
...
In a later post, OR/Ms will be discussed further, but for now I can say that the `ShoppingCartMap` and `ShoppingCartItemMap` classes are not necessary with Couchbase, the `Id` field from `Item` isn't necessary, and the Couchbase .NET SDK can directly populate a `ShoppingCart` object without an OR/M in a single line of code:
...
One way to model this is to have a document to represent a user, one document per update, and reference information to map a user to its friends and updates. This could be modeled in Couchbase using multiple documents. In this example, the `updates` field is an array of keys to other documents.
...
[forward reference, stay tuned]

There are ways to deal with these caveats in your application layer that will be discussed further in later blog posts in this series.
