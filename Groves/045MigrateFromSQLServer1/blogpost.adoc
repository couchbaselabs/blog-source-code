...
_I decided to model 'friends' as a one-way relationship (like Twitter) for this example, which is why Matt Groves has a friend while Nic Raboy has an empty friends array._
...
Note the array in the `Friends` field in the user document and the value in `User` field in the update document. These values can be used to retrieve the associated documents. Later in this post, I'll discuss how to do it with key/value operations and how to do it with N1QL.
