...
The ease or difficultly of a migration varies widely based on a number of factors. In some cases it may be extremely easy; in some cases it will be time-consuming and difficult; in some (shrinking number of) cases it may not even be a good idea.
...
This blog post covered data modeling using denormalization, data modeling using referencing, key design, and discriminating fields. Modeling data in a document database is a throught process, something of an art form, and not a mechanical process. There is no prescription to how to model your data in a document database: it depends greatly on how your application interacts with your data.
