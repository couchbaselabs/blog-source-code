...
The source code I used to generate the "patient" data used in this post link:[is available on GitHub]. If you aren't .NET savvy, you can just use link:https://developer.couchbase.com/documentation/server/current/tools/cbimport.html[cbimport] on link:[sample data] that I've created. (Or, you can use the N1QL sandbox, more information on that later). The rest of this blog post assumes you have a "patients" bucket with appropriate data in it.

== The data

Suppose I have a collection of patient documents. Each patient has a single doctor. The patient document refers to a doctor by a field called `doctorId`. There may be other data in the patient document, but we're mainly focused on the patient document's key and the `doctorId` value. Some examples:
...
Finally, what we want for our application (or report or whatever), is an aggregate grouping of the patients with their doctor. Something like:
...
Let's start with the query, and then break it down:
...
If you don't want to go through the trouble of creating a bucket and importing sample data, you can also try this in the link:http://query.pub.couchbase.com/tutorial/#1[N1QL tutorial sandbox]. There aren't patient documents in there, so the query will be a little different. I'm going to group up emails by age:
...
Similar aggregate grouping can also be achieved with a MapReduce index.
...
I think the N1QL method is easier, but there may be other benefits to using MapReduce. In either case, you can accomplish aggregate grouping just as easily (if not easier) as in a relational database.
