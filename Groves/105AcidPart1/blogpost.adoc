...
It's important to note that even with all these primitives, there's no way to get the same kind of total safety that a non-distributed database can offer with full ACID properties. But you get a different kind of safety: if a single non-distributed database goes down, then your application is going to stop working altogether. And if you need to scale your application up, you don't have to wrestle with the all the baggage of a rigidly ACID-compliant relational database system.
