...
The only schema that Couchbase needs is JSON. If you perform an insert operation with JSON, you must supply valid JSON (the Couchbase SDKs will typically handle this for you). This schema is only enforced at the document level. Couchbase cannot prevent you from using a different JSON schema from document to document. For instance, both of these documents can be inserted into the same bucket:
...
"Durability" traditionally means that when an operation completes successfully, that the changes the operation made are stored to disk. In a distributed database, durability can mean that changes are written to disk and/or other nodes in the system. Ultimately, to a developer, it means that even if a system failure occurs, the change still takes place.
