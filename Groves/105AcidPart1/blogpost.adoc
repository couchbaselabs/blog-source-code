...
It's important to note that even with all these primitives, there's no way to get the full ACID properties that a non-distributed database can offer. But, if a single non-distributed database goes down, then your application is going to stop working altogether. And if you need to scale your application up, you don't have to wrestle with the all the baggage of a rigidly ACID-compliant relational database system.

Applications have evolved from monolithic to micro-service based and they no longer rely on getting all of the ACID behaviors from the underlying database. Couchbase already provides a rich set of primitives for what you need. Using these, you can build to your own specifications. Couchbase is committed to keep enriching our transactional support.

Make sure you are using a distributed database for the right use case. Then you can get the performance, scalability, and flexibility of NoSQL when you need it.
