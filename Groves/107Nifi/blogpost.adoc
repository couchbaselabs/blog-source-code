...
The data team have access to a SQL Server database which is used to store live data about a game. Whenever a ticket is scanned at the gate, data is put into this database. Whenever concessions are purchased, data goes into this database. And so on.

The Reds could potentially query data directly from the SQL Server, but a real-time visualization during the heavy load of game time would just make it worse. Instead, they'd like to make a copy of that data into a Couchbase cluster, and use the Couchbase cluster as the backend for the visualization.
...
* link:https://hub.docker.com/r/apache/NiFi/[Docker image] for Apache NiFi
* link:https://hub.docker.com/r/microsoft/mssql-server-linux/[Microsoft SQL Server] (for Linux)
...
docker run -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=%pf84%EnrrsB9@' -p 1433:1433 -d microsoft/mssql-server-linux:2017-latest
----
...
I used SQL Server Management Studio to connect to the SQL Server instance in Docker (localost, port 1433). I don't (yet) have access to the Reds actual server, so I came up with my own schema to approximate what I expect to find there.
...
Later, I'll be populating it with data with an `INSERT` statement like so:
...
_The reason that you need to create a user with the same name is because the NiFi Couchbase processor is a little bit out of data, and hasn't been updated to handle the new RBAC capabilities of Couchbase. See Apache Nifi link:https://issues.apache.org/jira/browse/NIFI-5054[issue 5054] for more information._
...
If you used Docker, just visit *localhost:8080/NiFi*. You'll see what looks like a big sheet of graph paper and a toolbar at the top. Here's the complete data flow that I built:

image:10701-NiFi-end-state.png[NiFi complete data flow]

I'll go through this step-by-step, but keep in mind that I'm not a NiFi expert. But at a high level, each of these boxes are "processors". Each of them get data from somewhere, process the data in some way, and write the data somewhere else. This "flow" of data may come from an external source, a NiFi queue, or be written to an external source. Each processor can be "Started" and "Stopped".
...
* *Database User* - Enter a SQL Server user name. I created one named "matt".
...
* *Database Type* - I selected MS SQL 2008, which seems to work find with MS SQL for Linux, but there are also options for MS SQL 2012+ and "Generic".
...
The ConcertAvroToJSON processor does exactly what it says. This will prepare the Avro data for Couchbase. I changed the *JSON container options* property from `array` to `none`. I just want a plain JSON document, and not an array containing a single document.
...
In the above animation, I'm inserting two new documents to SQL Server. NiFi (not pictured) is processing them and putting them into Couchbase.
...
* The scaling capabilities of Couchbase allows you to increase your capabilities without having to take your flow offline

I'm just learning NiFi for the first time, and I'm already loving the graphical interface and the simplicity of getting started. I still have a lot to learn, but hopefully this post will help you out with using the Couchbase.
...
If you have questions about Couchbase, please check out the link:https://forums.couchbase.com/c/couchbase-server[Couchbase Server forums]. If you have questions about NiFi, check out the link:https://nifi.apache.org/[Apache project website].
